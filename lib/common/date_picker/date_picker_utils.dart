...
class DatePickerUtils {
  ...
  static List<String> englishDaysBetweenDates({
    ...
    final monthInt = _englishMonthIndex(currentMonth) + 1;
    if (minDate.year == currentYear &&
        currentYear == maxDate.year &&
        minDate.month == monthInt &&
        maxDate.month == monthInt) {
    ...
    } else if (currentYear <= minDate.year && monthInt <= minDate.month) {
      return List.generate((maxDay - minDate.day) + 1,
          (index) => (minDate!.day + index).toString());
    } else if (currentYear >= maxDate.year && monthInt >= maxDate.month) {
  ...
  static List<String> nepaliDaysBetweenDates({
    ...
    final monthInt = _nepaliMonthIndex(currentMonth) + 1;
    if (minDate.year == currentYear &&
        currentYear == maxDate.year &&
        minDate.month == monthInt &&
        maxDate.month == monthInt) {
  ...
  static int findEnglishLastDate(int year, String month) {
    ...
    final date = DateTime(year, monthIndex + 2, 0);
    return date.day;
  }
